...
Реализован класс и все его методы. Приведены примеры использования методов и проверна их корректность. Вы проверили пограничные случаи (прочность оружия 0, попытка уменьших прочность ниже 0, прочность оружия — бусконечно.). 

## Задача №2. Переработка оружия

После некотрых размышлений, Вася понял, что очень часто создает оружие из **таблицы 1** и **таблицы 2**. Он решил сделать для каждого из них свой класс.
